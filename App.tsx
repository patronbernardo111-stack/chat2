...
const App: React.FC = () => {
          ...
          return (
            ...
            <div className="chat-view-container" style={{ 
              ...
              <div
                ...
                style={{ flex: 1, minHeight: 0, overflowY: 'scroll', overflowX: 'hidden', WebkitOverflowScrolling: 'touch' as any, padding: '10px 10px 8px', paddingTop: device.isMobile ? 'calc(env(safe-area-inset-top, 44px) + 60px)' : '70px', display: 'flex', flexDirection: 'column', gap: '3px', position: 'relative', zIndex: 1, background: getActiveChatWallpaper() === 'none' ? '#efeae2' : 'transparent' }}
              >
                    ...
                    >
                      ...
                      {(msg as any).type === 'call' ? (() => {
                        ...
                        (msg.text?.startsWith('💸') || (msg as any).type === 'money') ? (() => {
                          // Tarjeta compacta estilo Alipay
                          const lines = (msg.text || '').split('\n');
                          ...
                          return (
                            <div style={{ width:'200px', borderRadius:'12px', overflow:'hidden', boxShadow:'0 2px 12px rgba(0,0,0,0.15)' }}>
                              {/* Header compacto */}
                              <div style={{ background: isSender ? 'linear-gradient(135deg,#1a73e8,#0d47a1)' : 'linear-gradient(135deg,#00c8a0,#00897b)', padding:'10px 12px 12px', position:'relative', overflow:'hidden' }}>
                                <div style={{ position:'absolute', top:'-15px', right:'-15px', width:'60px', height:'60px', borderRadius:'50%', background:'rgba(255,255,255,0.08)' }}/>
                                <div style={{ display:'flex', alignItems:'center', gap:'6px', marginBottom:'6px' }}>
                                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.8)" strokeWidth="2.5" strokeLinecap="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
                                  <span style={{ fontSize:'10px', color:'rgba(255,255,255,0.8)', fontWeight:'600' }}>{isSender ? 'Pago enviado' : 'Dinero recibido'}</span>
                                </div>
                                <div style={{ display:'flex', alignItems:'baseline', gap:'4px' }}>
                                  <span style={{ fontSize:'10px', color:'rgba(255,255,255,0.65)', fontWeight:'500' }}>XAF</span>
                                  <span style={{ fontSize:'22px', fontWeight:'800', color:'#fff', lineHeight:1 }}>{amountNum}</span>
                                </div>
                              </div>
                              {/* Body compacto */}
                              <div style={{ background:'#fff', padding:'8px 12px 10px' }}>
                                {(isSender ? to : from) ? (
                                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'4px' }}>
                                    <span style={{ fontSize:'10px', color:'#9ca3af' }}>{isSender ? 'Para' : 'De'}</span>
                                    <span style={{ fontSize:'12px', fontWeight:'700', color:'#111827' }}>{isSender ? to : from}</span>
                                  </div>
                                ) : null}
                                {ref ? (
                                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', paddingTop:'4px', borderTop:'1px solid #f3f4f6', marginBottom:'6px' }}>
                                    <span style={{ fontSize:'10px', color:'#9ca3af' }}>Ref.</span>
  ...
  return (
    ...
    >
      ...
      {/* Visor de imagen — componente completo con edición, recorte, reenvío */}
      {chatImageViewer && (
        <ImageViewer
          imageUrl={chatImageViewer}
          senderName={selectedChat?.title}
          sentAt={new Date().toLocaleDateString('es-ES', { day: '2-digit', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
          onClose={() => setChatImageViewer(null)}
          onDelete={() => { setChatImageViewer(null); }}
          onEdit={(editedUrl) => { setChatImageViewer(editedUrl); }}
          onForward={(contactIds) => {
            contactIds.forEach(cid => {
              const contact = allContacts.find((c: any) => c.id === cid);
              if (contact && chatImageViewer) {
                const msg = { id: `local-${Date.now()}-${cid}`, from: 'me' as const, text: '📷 Foto', time: new Date().toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' }), type: 'image' as const, imageUrl: chatImageViewer, status: 'pending' as const, created_at: new Date().toISOString() };
                setChatMessages((prev: any) => ({ ...prev, [cid]: [...(prev[cid] || []), msg] }));
              }
            });
            setChatImageViewer(null);
            showToast(`Reenviado a ${contactIds.length} contacto${contactIds.length > 1 ? 's' : ''}`, 'success');
          }}
          contacts={allContacts.map((c: any) => ({ id: c.id, title: c.full_name || c.name || c.title || 'Contacto', avatarUrl: c.avatar_url || c.avatarUrl, initials: (c.full_name || c.name || 'C').slice(0, 2).toUpperCase(), color: '#00c8a0' }))}
        />
      )}
      {/* Llamada entrante */}
