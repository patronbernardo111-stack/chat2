...
// ── SVG icons ─────────────────────────────────────────────────────
const IcoBanco = ({c='#1B3A6B'}:{c?:string}) => (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="10" width="18" height="11" rx="2"/><path d="M3 10l9-7 9 7"/><line x1="12" y1="10" x2="12" y2="21"/><line x1="7" y1="14" x2="7" y2="17"/><line x1="17" y1="14" x2="17" y2="17"/></svg>);
const IcoTransfer = ({c='#065F46'}:{c?:string}) => (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="M17 1l4 4-4 4"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><path d="M7 23l-4-4 4-4"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg>);
const IcoCodigo = ({c='#92400E'}:{c?:string}) => (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="16" rx="2"/><line x1="7" y1="9" x2="17" y2="9"/><line x1="7" y1="13" x2="13" y2="13"/><line x1="7" y1="17" x2="10" y2="17"/></svg>);
const IcoEfectivo = ({c='#4C1D95'}:{c?:string}) => (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="6" width="20" height="12" rx="2"/><circle cx="12" cy="12" r="3"/><path d="M6 12h.01M18 12h.01"/></svg>);
const IcoTarjeta = ({c='#1B3A6B'}:{c?:string}) => (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="5" width="20" height="14" rx="2"/><line x1="2" y1="10" x2="22" y2="10"/><line x1="6" y1="15" x2="10" y2="15"/></svg>);
const IcoAgente = ({c='#4C1D95'}:{c?:string}) => (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>);
...
const SectionHead = ({title,sub,color,icon}:{title:string;sub:string;color:string;icon:React.ReactNode}) => (
  <div style={{background:color,borderRadius:'16px',padding:'16px 18px',marginBottom:'16px',display:'flex',alignItems:'center',gap:'14px'}}>
    ...
    <div><div style={{fontSize:'15px',fontWeight:'800',color:'#fff'}}>{title}</div><div style={{fontSize:'11px',color:'rgba(255,255,255,0.75)',marginTop:'2px'}}>{sub}</div></div>
  </div>
);
...
export const WalletProvider: React.FC<{children: ReactNode; initialBalance?: number}> = ({ children, initialBalance = 125450 }) => {
  const [balance, setBalance] = useState<number>(() => {
    const saved = localStorage.getItem('wallet_balance');
    return saved ? parseInt(saved) : initialBalance;
  });
  ...
  useEffect(() => { localStorage.setItem('wallet_balance', balance.toString()); }, [balance]);
  useEffect(() => { localStorage.setItem('wallet_transactions', JSON.stringify(transactions)); }, [transactions]);

  const addBalance = (amount: number) => setBalance(p => p + amount);
  ...
  React.useEffect(() => {
    ...
    walletAPI.getTransactions()
      .then(r => { if (r?.transactions) setTransactions(r.transactions); })
      .catch(() => {});
  }, []);

  return (
    <WalletContext.Provider value={{ balance, addBalance, subtractBalance, transactions, addTransaction }}>
      {children}
...
export const RecargaMonederoModal: React.FC<{onClose:()=>void}> = ({onClose}) => {
  ...
  const processDeposit = async (amount:number, method:string, reference:string) => {
    try {
      const res = await walletAPI.deposit(amount, method, reference);
      // Ajustar saldo al valor del servidor
      const diff = res.balance - balance;
      if (diff > 0) addBalance(diff);
      addTransaction({type:'deposit', amount, method, reference, status:'completed'});
    } catch {
      addBalance(amount);
      addTransaction({type:'deposit', amount, method, reference, status:'completed'});
    }
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.45)',zIndex:4000,display:'flex',alignItems:'flex-end',justifyContent:'center'}} onClick={e=>{if(e.target===e.currentTarget)close();}}>
      <div style={{background:'rgba(247,248,250,0.55)',backdropFilter:'blur(28px) saturate(180%)',WebkitBackdropFilter:'blur(28px) saturate(180%)',borderRadius:'20px 20px 0 0',border:'1.5px solid rgba(255,255,255,0.6)',borderBottom:'none',width:'100%',maxWidth:'420px',maxHeight:'92vh',display:'flex',flexDirection:'column',overflow:'hidden'}}>
        ...
        <div style={{padding:'4px 16px 12px',display:'flex',alignItems:'center',gap:'10px',flexShrink:0,background:'#fff',borderBottom:'1px solid #F0F2F5'}}>
          ...
          <div style={{flex:1}}><div style={{fontSize:'16px',fontWeight:'700',color:'#111827'}}>{TITLES[step]}</div><div style={{fontSize:'11px',color:'#9CA3AF'}}>Saldo: {balance.toLocaleString()} XAF</div></div>
        ...
        <div style={{flex:1,overflowY:'auto',padding:'16px 16px 32px'}}>
          {step==='menu'&&<div>
            <SectionHead title="Monedero EGCHAT" sub={`${balance.toLocaleString()} XAF disponibles`} color="linear-gradient(135deg,#1A3A6B,#0E5F8A)" icon={<IcoTarjeta c="#fff"/>}/>
          ...
          {step==='banco'&&<div>
            ...
            <div style={{background:'#EFF5FD',borderRadius:'12px',padding:'14px',marginBottom:'14px',border:'1px solid #BFDBFE'}}>
              {[['Beneficiario','EGCHAT S.A.'],['Banco','BANGE / CCEI / BGFI'],['Cuenta','GQ-EGCHAT-001-2026'],['Concepto','Recarga EGCHAT + tu teléfono']].map(([l,v])=>(
                <div key={l} style={{display:'flex',justifyContent:'space-between',padding:'5px 0',borderBottom:'1px solid #DBEAFE'}}><span style={{fontSize:'11px',color:'#6B7280'}}>{l}</span><span style={{fontSize:'11px',fontWeight:'700',color:'#1B3A6B'}}>{v}</span></div>
              ))}
            </div>
          ...
          {step==='transferencia'&&<div>
            ...
            <div style={{background:'#fff',borderRadius:'14px',padding:'20px',marginBottom:'14px',textAlign:'center',border:'1px solid #F0F2F5'}}>
              <div style={{width:'100px',height:'100px',margin:'0 auto 12px',background:'#F0FDF9',borderRadius:'16px',display:'flex',alignItems:'center',justifyContent:'center'}}>
                <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#00c8a0" strokeWidth="1.4"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="3" height="3"/><rect x="18" y="14" width="3" height="3"/><rect x="14" y="18" width="3" height="3"/><rect x="18" y="18" width="3" height="3"/></svg>
              </div>
          ...
          {step==='codigo'&&<div>
            ...
            <button onClick={async ()=>{
              if(data.codigo&&data.codigo.replace(/-/g,'').length===16){
                try {
                  const res = await walletAPI.redeemCode(data.codigo);
                  const diff = res.amount;
                  addBalance(diff);
                  addTransaction({type:'deposit', amount:res.amount, method:'Código de recarga', reference:data.codigo, status:'completed'});
                  set('amount', String(res.amount));
                } catch {
                  const amount = parseInt(data.amount||'5000');
                  processDeposit(amount,'Código de recarga',data.codigo);
                }
                setStep('success');
              }
            }} style={{width:'100%',background:data.codigo&&data.codigo.replace(/-/g,'').length===16?'linear-gradient(135deg,#92400E,#D97706)':'#E5E7EB',border:'none',borderRadius:'12px',padding:'14px',color:data.codigo&&data.codigo.replace(/-/g,'').length===16?'#fff':'#9CA3AF',fontSize:'14px',fontWeight:'700',cursor:data.codigo&&data.codigo.replace(/-/g,'').length===16?'pointer':'default'}}>Canjear código</button>
          </div>}
          {step==='efectivo'&&<div>
            ...
            {[{nombre:'Agente EGCHAT Centro',dir:'Av. de la Independencia, Malabo',horario:'L-S 8:00-20:00',tel:'+240 222 30 00 01'},{nombre:'Agente EGCHAT Caracolas',dir:'Barrio Caracolas, Malabo',horario:'L-D 8:00-21:00',tel:'+240 222 30 00 02'},{nombre:'Agente EGCHAT Ela Nguema',dir:'Ela Nguema, Malabo',horario:'L-S 8:00-19:00',tel:'+240 222 30 00 03'},{nombre:'Agente EGCHAT Bata Centro',dir:'Centro de Bata',horario:'L-D 8:00-21:00',tel:'+240 222 30 00 04'}].map(a=>(
              <div key={a.nombre} style={{background:'#fff',borderRadius:'12px',padding:'13px 14px',marginBottom:'8px',border:'1px solid #F0F2F5',display:'flex',alignItems:'center',gap:'10px'}}>
                ...
                <a href={`tel:${a.tel}`} style={{background:'#F5F3FF',border:'none',borderRadius:'8px',padding:'7px 10px',fontSize:'11px',fontWeight:'700',color:'#4C1D95',textDecoration:'none'}}>📞</a>
              </div>
            ))}
            <Field k="amount" l="Importe a depositar (XAF)" t="number" icon={<IcoEfectivo c="#9CA3AF"/>}/>
            <button onClick={()=>{if(isValid){processDeposit(amountNum,'Depósito en efectivo (Agente)','Agente presencial');setStep('success');}}} style={{width:'100%',background:isValid?'linear-gradient(135deg,#4C1D95,#6B5BD6)':'#E5E7EB',border:'none',borderRadius:'12px',padding:'14px',color:isValid?'#fff':'#9CA3AF',fontSize:'14px',fontWeight:'700',cursor:isValid?'pointer':'default',marginTop:'8px'}}>
              Confirmar depósito {isValid?`· ${amountNum.toLocaleString()} XAF`:''}
            </button>
          </div>}
          {step==='confirm'&&<div>
            ...
            <div style={{background:'#fff',borderRadius:'14px',padding:'16px',marginBottom:'14px',border:'1px solid #F0F2F5'}}>
              {[['Método','Transferencia bancaria'],['Banco',data.banco||''],['Importe',`${amountNum.toLocaleString()} XAF`],['Cuenta destino','GQ-EGCHAT-001-2026']].map(([l,v])=>(
                <div key={l} style={{display:'flex',justifyContent:'space-between',padding:'7px 0',borderBottom:'1px solid #F3F4F6'}}><span style={{fontSize:'12px',color:'#6B7280'}}>{l}</span><span style={{fontSize:'12px',fontWeight:'700',color:'#111827'}}>{v}</span></div>
              ))}
              <div style={{display:'flex',justifyContent:'space-between',paddingTop:'10px'}}><span style={{fontSize:'14px',fontWeight:'700',color:'#374151'}}>Total</span><span style={{fontSize:'20px',fontWeight:'900',color:'#1B3A6B'}}>{amountNum.toLocaleString()} XAF</span></div>
            </div>
            <button onClick={()=>{processDeposit(amountNum,'Transferencia bancaria',`Banco: ${data.banco}`);setStep('success');}} style={{width:'100%',background:'linear-gradient(135deg,#1B3A6B,#0E5F8A)',border:'none',borderRadius:'12px',padding:'14px',color:'#fff',fontSize:'14px',fontWeight:'700',cursor:'pointer'}}>He realizado la transferencia</button>
          </div>}
          {step==='success'&&<div style={{textAlign:'center',padding:'30px 0'}}>
            ...
            <div style={{fontSize:'20px',fontWeight:'900',color:'#111827',marginBottom:'6px'}}>{data.codigo?'¡Código canjeado!':data.remitente?'¡Solicitud enviada!':'¡Recarga completada!'}</div>
            <div style={{fontSize:'13px',color:'#9CA3AF',marginBottom:'20px',lineHeight:'1.5',padding:'0 16px'}}>{data.codigo?`+${parseInt(data.amount||'5000').toLocaleString()} XAF añadidos`:data.remitente?'El usuario recibirá tu solicitud':`${amountNum.toLocaleString()} XAF añadidos a tu monedero`}</div>
            <div style={{background:'#F0FDF9',borderRadius:'14px',padding:'16px',marginBottom:'20px',textAlign:'left',border:'1px solid #A7F3D0'}}>
              ...
              <div style={{display:'flex',justifyContent:'space-between',marginTop:'10px',paddingTop:'10px',borderTop:'1px solid #D1FAE5'}}><span style={{fontSize:'12px',color:'#6B7280'}}>Nuevo saldo</span><span style={{fontSize:'16px',fontWeight:'900',color:'#00c8a0'}}>{balance.toLocaleString()} XAF</span></div>
            </div>
...
export const RetiroMonederoModal: React.FC<{onClose:()=>void}> = ({onClose}) => {
  const {balance, subtractBalance, addTransaction} = useWallet();
  ...
  const processWithdraw = async (amount:number, method:string, destination:string) => {
    try {
      await walletAPI.withdraw(amount, method, destination);
      subtractBalance(amount);
      addTransaction({type:'withdraw', amount, method, reference:destination, status:'completed'});
    } catch {
      subtractBalance(amount);
      addTransaction({type:'withdraw', amount, method, reference:destination, status:'completed'});
    }
  ...
  const InsufficientAlert = () => amountNum > balance && amountNum > 0 ? (
    <div style={{background:'#FEF2F2',borderRadius:'10px',padding:'10px 14px',marginBottom:'8px',fontSize:'11px',color:'#C0392B',fontWeight:'600',display:'flex',gap:'6px'}}>
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.45)',zIndex:4000,display:'flex',alignItems:'flex-end',justifyContent:'center'}} onClick={e=>{if(e.target===e.currentTarget)close();}}>
      <div style={{background:'rgba(247,248,250,0.55)',backdropFilter:'blur(28px) saturate(180%)',WebkitBackdropFilter:'blur(28px) saturate(180%)',borderRadius:'20px 20px 0 0',border:'1.5px solid rgba(255,255,255,0.6)',borderBottom:'none',width:'100%',maxWidth:'420px',maxHeight:'92vh',display:'flex',flexDirection:'column',overflow:'hidden'}}>
        ...
        <div style={{padding:'4px 16px 12px',display:'flex',alignItems:'center',gap:'10px',flexShrink:0,background:'#fff',borderBottom:'1px solid #F0F2F5'}}>
          <button onClick={goBack} style={{background:'#EAECEF',border:'none',borderRadius:'50%',width:'32px',height:'32px',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',color:'#6B7280',fontSize:'16px'}}>←</button>
          <div style={{flex:1}}><div style={{fontSize:'16px',fontWeight:'700',color:'#111827'}}>{TITLES[step]}</div><div style={{fontSize:'11px',color:'#9CA3AF'}}>Disponible: {balance.toLocaleString()} XAF</div></div>
          <button onClick={close} style={{background:'#EAECEF',border:'none',borderRadius:'50%',width:'30px',height:'30px',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',color:'#6B7280',fontSize:'14px'}}>✕</button>
        </div>
        <div style={{flex:1,overflowY:'auto',padding:'16px 16px 32px'}}>
          {step==='menu'&&<div>
            <SectionHead title="Retirar dinero" sub={`${balance.toLocaleString()} XAF disponibles`} color="linear-gradient(135deg,#4C1D95,#6B5BD6)" icon={<IcoEfectivo c="#fff"/>}/>
          ...
          {step==='tarjeta'&&<div>
            ...
            <Field k="pin" l="PIN de confirmación" t="password" icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#9CA3AF" strokeWidth="1.7"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>}/>
            <InsufficientAlert/>
          ...
          {step==='banco'&&<div>
            ...
            <Field k="cuenta" l="Número de cuenta bancaria" t="text" icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#9CA3AF" strokeWidth="1.7"><rect x="3" y="4" width="18" height="16" rx="2"/><line x1="7" y1="9" x2="17" y2="9"/><line x1="7" y1="13" x2="13" y2="13"/></svg>}/>
            <Field k="titular" l="Titular de la cuenta" t="text" icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#9CA3AF" strokeWidth="1.7"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>}/>
            <InsufficientAlert/>
          ...
          {step==='agente'&&<div>
            ...
            <Field k="amount" l="Importe a retirar (mín. 1,000 XAF)" t="number" icon={<IcoEfectivo c="#9CA3AF"/>}/>
            <InsufficientAlert/>
            {[{nombre:'Agente EGCHAT Centro',dir:'Av. de la Independencia, Malabo',tel:'+240 222 30 00 01'},{nombre:'Agente EGCHAT Caracolas',dir:'Barrio Caracolas, Malabo',tel:'+240 222 30 00 02'},{nombre:'Agente EGCHAT Bata Centro',dir:'Centro de Bata',tel:'+240 222 30 00 04'}].map(a=>(
          ...
          {step==='confirm'&&<div>
            ...
            <div style={{background:'#fff',borderRadius:'14px',padding:'16px',marginBottom:'14px',border:'1px solid #F0F2F5'}}>
              {[['Importe',`${amountNum.toLocaleString()} XAF`],['Método',data.cardNumber?'Tarjeta bancaria':data.cuenta?'Transferencia bancaria':'Agente EGCHAT'],['Saldo tras retiro',`${(balance-amountNum).toLocaleString()} XAF`]].map(([l,v])=>(
                <div key={l} style={{display:'flex',justifyContent:'space-between',padding:'7px 0',borderBottom:'1px solid #F3F4F6'}}><span style={{fontSize:'12px',color:'#6B7280'}}>{l}</span><span style={{fontSize:'12px',fontWeight:'700',color:'#111827'}}>{v}</span></div>
              ))}
              <div style={{display:'flex',justifyContent:'space-between',paddingTop:'10px'}}><span style={{fontSize:'14px',fontWeight:'700',color:'#374151'}}>Total a retirar</span><span style={{fontSize:'20px',fontWeight:'900',color:'#C0392B'}}>{amountNum.toLocaleString()} XAF</span></div>
            </div>
            <div style={{background:'#FEF2F2',borderRadius:'10px',padding:'10px 14px',marginBottom:'14px',fontSize:'11px',color:'#C0392B',display:'flex',gap:'8px'}}>
              ...
              <span>Esta operación debitará {amountNum.toLocaleString()} XAF. No se puede deshacer.</span>
            </div>
            <button onClick={()=>{const method=data.cardNumber?'Tarjeta bancaria':data.cuenta?'Transferencia bancaria':'Retiro en agente';const dest=data.cardNumber||`${data.banco}-${data.cuenta}`||'Código agente';processWithdraw(amountNum,method,dest);setStep('success');}} style={{width:'100%',background:'linear-gradient(135deg,#C0392B,#E74C3C)',border:'none',borderRadius:'12px',padding:'14px',color:'#fff',fontSize:'14px',fontWeight:'700',cursor:'pointer',marginBottom:'8px'}}>
          ...
          {step==='success'&&<div style={{textAlign:'center',padding:'30px 0'}}>
            ...
            <div style={{background:'#F5F3FF',borderRadius:'14px',padding:'16px',marginBottom:'20px',textAlign:'left',border:'1px solid #DDD6FE'}}>
              ...
              <div style={{display:'flex',justifyContent:'space-between',marginTop:'10px',paddingTop:'10px',borderTop:'1px solid #DDD6FE'}}><span style={{fontSize:'12px',color:'#6B7280'}}>Nuevo saldo</span><span style={{fontSize:'16px',fontWeight:'900',color:'#4C1D95'}}>{balance.toLocaleString()} XAF</span></div>
            </div>
